...
do
	...
	do
		...
		#(time ./e_test_x86 $i) 2>> res2.txt
		(time ./e_test_x86_mpfr $i) 2>> res_mpfr.txt
	done
done

cat res.txt | grep real > x86.txt
#cat res2.txt | grep real > x86_2.txt
cat res_mpfr.txt | grep real > x86_mpfr.txt

sed -e "s/real.*m//" -e "s/s/ /" -e "s/,/./" x86.txt > x86_e_float.txt
#sed -e "s/real.*m//" -e "s/s/ /" -e "s/,/./" x86_2.txt > x86_e_double.txt
